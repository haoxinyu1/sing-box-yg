  ...
  reading "\n确定要卸载吗？【y/n】: " choice
    case "$choice" in
	  ...
	  sed -i '' '/export PATH="\$HOME\/bin:\$PATH"/d' ~/.bashrc
          ...
          purple "欢迎继续使用脚本：bash <(curl -Ls https://raw.githubusercontent.com/haoxinyu1/sing-box-yg/main/serv00.sh)"
          purple "************************************************************"
          ;;
        [Nn]) exit 0 ;;
...
if [[ -e $WORKDIR/config.json ]]; then
  ...
  mkdir -p "$HOME/bin"
  curl -Ls https://raw.githubusercontent.com/haoxinyu1/sing-box-yg/main/serv00.sh > "$SCRIPT_PATH"
  chmod +x "$SCRIPT_PATH"
...
latestV=$(curl -sL https://raw.githubusercontent.com/haoxinyu1/sing-box-yg/main/sversion | awk -F "更新内容" '{print $1}' | head -n 1)
if [ -f $WORKDIR/v ]; then
...
echo -e "${yellow}$(curl -sL https://raw.githubusercontent.com/haoxinyu1/sing-box-yg/main/sversion)${re}"
fi
echo -e "========================================================="
